...
/** 确定性直出只对已证明单调的棋盘启用。 */
export const supportsForcedExitClosure = (board) => board.model === "facing" && board.slideMode === "all"
    && board.returning.size === 0
    && [...board.pieces.values()].every((p) => CLOSURE_SPECIES.has(p.species));
/** 沿朝向依次遇到的其它棋子（去重保序）。 */
function forwardOwners(board, pieceId, direction) {
...
function protectedExitStoppers(board, legalMoves) {
    const exitIds = new Set(legalMoves.filter((m) => m.result === "EXIT").map((m) => m.pieceId));
    if (!exitIds.size)
        return new Set();
    // 把当前可直出的棋子从依赖图中折叠掉，剩下的边就是急切闭包
    // 清空所有出口后每只棋子最终会停靠的阻挡者。
    const collapsedBlocker = new Map();
    for (const [pid, piece] of board.pieces) {
        ...
        if (blocker !== undefined)
            collapsedBlocker.set(pid, blocker);
    }
    const protectedIds = new Set();
    for (const move of legalMoves) {
        if (move.result === "EXIT" || move.distance <= 0)
            continue;
        const stagedExits = [];
        let finalBlocker = null;
        for (const owner of forwardOwners(board, move.pieceId, move.direction)) {
            if (exitIds.has(owner))
                stagedExits.push(owner);
            else {
                finalBlocker = owner;
                break;
            }
        }
        if (!stagedExits.length || finalBlocker === null)
            continue;
        let current = finalBlocker;
        const seen = new Set();
        while (current !== undefined && !seen.has(current)) {
            if (current === move.pieceId) {
                for (const pid of stagedExits)
...
function exitUnlockCount(board, move) {
    if (board.model !== "facing")
        return 0;
    const target = new Set(board.pieces.get(move.pieceId).cells);
    let count = 0;
    for (const [pid, piece] of board.pieces) {
        if (pid === move.pieceId || !piece.facing)
            continue;
        const delta = DELTA[piece.facing];
        if (piece.cells.some((c) => target.has((c + delta))))
            count += 1;
    }
    return count;
}
export const forcedExitSortKey = (board, move) => [-exitUnlockCount(board, move), move.anchor[0], move.anchor[1], move.pieceId];
/** 当前状态下可安全确定性清除的直出动作。 */
export function forcedExitCandidates(board, { ordinaryOnly = false } = {}) {
    const legalMoves = board.legalMoves();
    const protectedIds = protectedExitStoppers(board, legalMoves);
    return legalMoves.filter((move) => move.result === "EXIT"
        && !protectedIds.has(move.pieceId)
        && (!ordinaryOnly || board.pieces.get(move.pieceId)?.species === "sheep"));
}
/** 只清除"止挡安全"的直出并返回规范化棋盘。 */
export function forcedExitClosure(board) {
