 ...
 */
import { DELTA, cell, colOf, rowOf } from "./types.js";
/** owners() 映射中危险格的占位所有者。 */
export const HAZARD = "#hazard";
function normalizePiece(data, defaultSpecies = "sheep") {
...
const allowedDirs = (piece, model) => {
    if (piece.species === "pig" && !piece.awake)
        return [];
    if (model === "facing") {
        if (!piece.facing)
            throw new Error("facing 模型要求每只羊带 facing 字段");
        return [piece.facing];
    }
...
export class Board {
    ...
    static from(data, extra = {}) {
        ...
        return new Board({
            ...
            fences: new Set((data.fences ?? []).map((f) => `${f.cell[0]},${f.cell[1]},${f.direction}`)),
            pieces,
            returning,
        });
    }
    ...
    /** 离场瞬间穿过的边界栅栏段。 */
    fenceCrossings(frontier, direction) {
        const delta = DELTA[direction];
        const hits = new Set();
        for (const c of frontier) {
            if (!this.inBoard((c + delta))) {
                const key = `${rowOf(c)},${colOf(c)},${direction}`;
                if (this.fences.has(key))
                    hits.add(key);
            }
        }
        return hits;
    ...
    /** 下一格推进将进入的内部 H/V 栅栏格。 */
    fenceCellHits(frontier, direction) {
        const delta = DELTA[direction];
        const hits = new Set();
        for (const c of frontier) {
            const next = (c + delta);
    ...
    #collisionIsSafe(movingPid, frontier, direction) {
        const delta = DELTA[direction];
        ...
        if (this.pieces.get(movingPid).species === "bomb")
            affected.add(movingPid);
        for (const pid of affected) {
            const piece = this.pieces.get(pid);
            if (piece.species === "bomb" && (piece.hitsRemaining || 3) <= 1)
                return false;
        }
        return true;
    }
    /** 与指定棋子边/角相邻的其它棋子（粉羊离场携带判定）。 */
    #touching(movingPid, cells) {
    ...
    #slide(pid, piece, d, owners) {
        ...
        const first = cells[0];
        const anchor = [rowOf(first), colOf(first)];
        const mk = (result, distance) => ({ pieceId: pid, direction: d, anchor, result, distance });
        if (exited)
            return mk("EXIT", 0);
        if (steps === 0)
            return null;
        ...
        if (!this.#collisionIsSafe(pid, frontier, d))
            return null;
        ...
        for (let k = 1; k <= steps; k++) {
            if (!cells.some((c) => this.noStop.has((c + delta * k)))) {
                out.push(mk("MOVE", k));
            }
        }
        return out;
    ...
    apply(mv) {
        ...
        let returning = new Map(this.returning);
        ...
        if (moving?.species === "cattle" && mv.result === "EXIT") {
            ...
            for (;;) {
                ...
                if (internalHits.size) {
                    for (const seg of this.internalFenceRun(internalHits))
                        broken.delete(seg);
                    break;
                }
                const next = frontier.map((c) => (c + delta));
                if (next.some((c) => !this.inBoard(c))) {
                    for (const seg of this.fenceCrossings(frontier, mv.direction))
                        broken.delete(seg);
                    break;
                }
                frontier = next;
        ...
        switch (mv.result) {
            ...
            case "EXIT": {
                ...
                if (piece.species === "pink_sheep") {
                    for (const pid of this.#touching(mv.pieceId, piece.cells))
                        pieces.delete(pid);
                }
                break;
            }
            default: {
                ...
                const piece = pieces.get(mv.pieceId);
                const moved = piece.cells.map((c) => (c + delta));
                ...
                if (mv.result === "MOVE") {
                    ...
                    for (const c of moved) {
                        const owner = owners.get((c + step));
                        if (owner !== undefined)
                            affected.add(owner);
                    }
                    if (pieces.get(mv.pieceId).species === "bomb")
                        affected.add(mv.pieceId);
                    for (const pid of affected) {
