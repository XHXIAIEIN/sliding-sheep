...
import { DELTA, lexCompare } from "./types.js";
import { Board, HAZARD } from "./board.js";
import { forcedExitClosure, supportsForcedExitClosure } from "./closure.js";
import { heuristic, rank, rankCompare } from "./heuristics.js";
export const now = () => (globalThis.performance?.now() ?? Date.now()) / 1000;
    ...
    pop() {
        ...
        if (items.length) {
            ...
            for (;;) {
                ...
                let m = i;
                if (l < items.length && lexCompare(prio[l], prio[m]) < 0)
                    m = l;
                if (r < items.length && lexCompare(prio[r], prio[m]) < 0)
                    m = r;
                if (m === i)
                    break;
...
const moveRank = (mv) => [mv.result === "EXIT" ? 0 : 1, -mv.distance, mv.pieceId];
const orderedMoves = (board) => board.legalMoves().sort((a, b) => lexCompare(moveRank(a), moveRank(b)));
/** A* 最少点击解；启发式 h = 剩余羊数（可采纳）。无解时返回 moves=null。 */
export function exactSolve(board, { maxNodes = 400_000, signal, } = {}) {
    let counter = 0;
    const heap = new Heap();
    ...
    while (heap.size) {
        if (signal?.aborted)
            return [null, { reason: "已取消", expanded, cancelled: true }];
        const [cur, path] = heap.pop();
        const g = path.length;
        if (cur.isSolved())
            return [path, { moves: path.length, expanded }];
        if (g > (bestG.get(cur.key()) ?? g))
            continue;
        expanded += 1;
        if (expanded > maxNodes)
            return [null, { reason: "超出搜索节点上限", expanded }];
...
export function greedySolve(board, { maxSteps = 10_000, signal, } = {}) {
    const features = (b) => {
        ...
        for (const [pid, piece] of b.pieces) {
            ...
            const firstBlocked = piece.cells.some((c) => {
                const owner = owners.get((c + delta));
            ...
            for (;;) {
                ...
                for (const c of next) {
                    ...
                    if (owner !== undefined && owner !== pid && owner !== HAZARD)
                        seen.add(owner);
                }
...
export function weightedAstarSolve(board, { weight = 1.35, maxNodes = 120_000, timeLimit = 8.0, signal, } = {}) {
    const start = now();
    ...
    while (heap.size && expanded < maxNodes && now() - start < timeLimit
        ...
        for (const mv of orderedMoves(cur)) {
            ...
            if (ng >= (bestG.get(key) ?? Infinity) || endgameUnsat.has(key))
                continue;
            if (next.remaining() <= 8) {
...
export function beamSolve(board, { width = 5000, maxDepth = 220, timeLimit = 12.0, seenCap = 450_000, signal, } = {}) {
    const start = now();
    const [prefix, closed] = forcedExitClosure(board);
    if (closed.isSolved()) {
    ...
    const nodes = [[0, null]];
    let beam = [[rank(closed, 0), closed, 0]];
...
export function randomizedMacroSolve(board, { seed = 0, timeLimit = 4.0, signal, } = {}) {
    ...
    while (now() < deadline && !signal?.aborted) {
        ...
        while (now() < deadline && !signal?.aborted) {
            ...
            if (rng() < 0.3) {
                ...
                const poolSize = Math.min(candidates.length, Math.max(2, Math.floor(Math.sqrt(candidates.length)) + 1));
                // 头部指数衰减的加权抽样。
                let total = 0;
                ...
                let index = 0;
                while (index < poolSize - 1 && (target -= weights[index]) > 0)
                    index += 1;
